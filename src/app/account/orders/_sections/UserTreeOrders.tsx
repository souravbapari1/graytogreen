...
import { getUserOrderTreeHistoryRequest } from "@/request/worker/account/ordersRequest";
import { useMutation } from "@tanstack/react-query";
import { useSession } from "next-auth/react";
import { useEffect, useState } from "react";
import OrderCard from "./OrderCard";

function UserTreeOrders() {
  const session = useSession();
  const [page, setPage] = useState(1);
  const loadAllOrders = useMutation({
    mutationKey: ["user-tree-orders", session.data?.user?.id, page],
    mutationFn: async () => {
      const data = await getUserOrderTreeHistoryRequest(
